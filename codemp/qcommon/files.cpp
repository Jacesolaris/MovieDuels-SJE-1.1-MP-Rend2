	...
	do
	{
		...
		for (const searchpath_t* search = fs_searchpaths; search; search = search->next) {
			...
			if (search->pack && search->pack->hashTable[hash]) {
				...
				// autoexec_mp.cfg and MovieDuels-SP.cfg can only be loaded outside of pk3 files.
				if (isUserConfig)
				{
					continue;
				}
					...
					if (!FS_FilenameCompare(pakFile->name, filename)) {
						...
						if (!(pak->referenced & FS_GENERAL_REF)) {
							if (!FS_IsExt(filename, ".shader", l) &&
								...
								Q_stricmp(filename, "MovieDuels-mpgamex86.dll") != 0 &&
...
static void FS_Startup(const char* gameName) {
	...
	//fs_gamedirvar = Cvar_Get("fs_game", "MD-MP", CVAR_INIT | CVAR_SYSTEMINFO, "Mod directory");
...
void FS_InitFilesystem(void) {
	...
	// if we can't find MD-MP-default.cfg, assume that the paths are
	// busted and error out now, rather than getting an unreadable
	// graphics screen when the font fails to load
	if (FS_ReadFile("MD-MP-default.cfg", nullptr) <= 0) {
		Com_Error(ERR_FATAL, "Couldn't load MD-MP-default.cfg");
...
void FS_Restart(int checksumFeed) {
	...
	if (Q_stricmp(fs_gamedirvar->string, lastValidGame)) {
		// skip the MovieDuels-mpconfig.cfg if "safe" is on the command line
		if (!Com_SafeMode()) {
