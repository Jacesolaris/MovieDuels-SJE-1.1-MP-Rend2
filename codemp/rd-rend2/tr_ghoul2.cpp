...
static void ProcessModelBoltSurfaces(
	int surfaceNum,
	surfaceInfo_v& rootSList,
	mdxaBone_v& bonePtr,
	model_t* currentModel,
...
static void* G2_FindSurface_BC(const model_s* mod, int index, int lod)
{
...
// We've come across a surface that's designated as a bolt surface, process it and put it in the appropriate bolt place
static void G2_ProcessSurfaceBolt2(
	CBoneCache& boneCache,
...
void RB_SurfaceGhoul(CRenderableSurface* surf)
{
	...
	if (surf->alternateTex)
	{
		...
		num_vertexes = surf->alternateTex->numVerts;
...
qboolean R_LoadMDXM(model_t* mod, void* buffer, const char* mod_name, qboolean& bAlreadyCached)
{
	...
	for (l = 0; l < mdxm->numLODs; l++)
	{
		...
		for (int n = 0; n < mdxm->numSurfaces; n++)
		{
			...
			vboMeshes[n].num_vertexes = surf->numVerts;
...
static int BoneParentChildIndex(
	mdxaHeader_t* mdxa,
	mdxaSkelOffsets_t* offsets,
	mdxaSkel_t* boneInfo,
	qboolean child)
