				...
				if (animSize)
				{
						...
						((animSpeed < 0.0f) && (newFrame_g < endFrame + 1)))
					{
						...
						if (blist[i].flags & BONE_ANIM_OVERRIDE_LOOP)
						{
							...
							blist[i].startTime = current_time - (int)((frameTime / animSpeed) * 50.0f);
...
static int G2_Set_Bone_Rag(const mdxaHeader_t* mod_a, boneInfo_v& blist, const char* boneName, CGhoul2Info& ghoul2, const vec3_t scale, const vec3_t origin)
{
...
static qboolean G2_Set_Bone_Anim_No_BS(CGhoul2Info& ghoul2, const mdxaHeader_t* mod, boneInfo_v& blist, const char* boneName, const int argStartFrame, const int argEndFrame, const int flags, const float animSpeed, const int current_time, const float set_frame, const int blendTime, const int creationID, bool resetBonemap = true)
{
	int			index = G2_Find_Bone_Rag(&ghoul2, blist, boneName);
	int			modFlags = flags;
...
static void G2_SetRagDollBullet(CGhoul2Info& ghoul2, const vec3_t rayStart, const vec3_t hit)
{
...
static inline char* G2_Get_Bone_Name(CGhoul2Info* ghlInfo, boneInfo_v& blist, int bone_num)
{
	mdxaSkel_t* skel;
	mdxaSkelOffsets_t* offsets;
	offsets = (mdxaSkelOffsets_t*)((byte*)ghlInfo->aHeader + sizeof(mdxaHeader_t));
...
void G2_InitIK(CGhoul2Info_v& ghoul2_v, sharedRagDollUpdateParams_t* parms, const int time, const mdxaHeader_t* mod_a, const int model)
{
	CGhoul2Info& ghoul2 = ghoul2_v[model];
	int curTime = time;
	boneInfo_v& blist = ghoul2.mBlist;
