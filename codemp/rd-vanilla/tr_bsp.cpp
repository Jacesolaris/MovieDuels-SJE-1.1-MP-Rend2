...
===============
*/
static shader_t* ShaderForShaderNum(int shader_num, const int* lightmap_num, const byte* lightmap_styles, const byte* vertex_styles, const world_t& worldData)
{
	const byte* styles = lightmap_styles;

	LL(shader_num);
	if (shader_num < 0 || shader_num >= worldData.numShaders) {
		Com_Error(ERR_DROP, "ShaderForShaderNum: bad num %i", shader_num);
	}
	const dshader_t* dsh = &worldData.shaders[shader_num];
...
static void ParseFace(const dsurface_t* ds, mapVert_t* verts, msurface_t* surf, int* indexes, const world_t& worldData, const int index) {
	...
	// get shader value
	surf->shader = ShaderForShaderNum(ds->shader_num, lightmap_num, ds->lightmapStyles, ds->vertexStyles, worldData);
...
static void ParseTriSurf(const dsurface_t* ds, mapVert_t* verts, msurface_t* surf, int* indexes, const world_t& worldData, const int index) {
	...
	// get shader
	surf->shader = ShaderForShaderNum(ds->shader_num, lightmapsVertex, ds->lightmapStyles, ds->vertexStyles, worldData);
	if (r_singleShader->integer && !surf->shader->sky) {
		surf->shader = tr.defaultShader;
	}
...
static void ParseFlare(const dsurface_t* ds, msurface_t* surf, const world_t& worldData, const int index) {
	...
	// get shader
	surf->shader = ShaderForShaderNum(ds->shader_num, lightmaps, ds->lightmapStyles, ds->vertexStyles, worldData);
	if (r_singleShader->integer && !surf->shader->sky) {
		surf->shader = tr.defaultShader;
	}
