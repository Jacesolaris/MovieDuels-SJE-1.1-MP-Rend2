...
qboolean G2_Set_Bone_Angles_Matrix(const char* file_name, boneInfo_v& blist, const char* boneName, const mdxaBone_t& matrix, const int flags, const qhandle_t* model_list, const int modelIndex, const int blendTime, const int current_time)
{
	...
	if (!file_name[0])
	{
		mod_m = R_GetModelByHandle(model_list[modelIndex]);
	}
	else
	{
...
qboolean G2_Set_Bone_Anim_Index(boneInfo_v& blist, const int index, const int startFrame, const int endFrame, const int flags, const float animSpeed, const int current_time, const float set_frame, const int ablend_time, const int numFrames)
{
	...
	if (set_frame != -1)
	{
		assert(set_frame >= startFrame && set_frame <= endFrame);
	}
	if (flags & BONE_ANIM_BLEND)
	{
		float current_frame, animSpeed;
		int startFrame, end_frame1, flags1;
		// figure out where we are now
		if (G2_Get_Bone_Anim_Index(blist, index, current_time, &current_frame, &startFrame, &end_frame1, &flags1, &animSpeed, NULL, numFrames))
		{
	...
	if (set_frame != -1)
	{
		blist[index].lastTime = blist[index].startTime = current_time - (set_frame - static_cast<float>(startFrame)) * 50.0 / animSpeed;
	}
	else
	{
...
qboolean G2_Set_Bone_Anim(const CGhoul2Info* ghlInfo, boneInfo_v& blist, const char* boneName, const int startFrame, const int endFrame, const int flags, const float animSpeed, const int current_time, const float set_frame, const int blendTime)
{
	...
	if (index != -1)
	{
		return G2_Set_Bone_Anim_Index(blist, index, startFrame, endFrame, flags, animSpeed, current_time, set_frame, blendTime, ghlInfo->aHeader->numFrames);
	}
	return qfalse;
}
...
qboolean G2_Get_Bone_Anim(const CGhoul2Info* ghlInfo, boneInfo_v& blist, const char* boneName, const int current_time, float* current_frame, int* startFrame, int* endFrame, int* flags, float* ret_anim_speed, qhandle_t* model_list, int modelIndex)
{
	...
	if (G2_Get_Bone_Anim_Index(blist, index, current_time, current_frame, startFrame, endFrame, flags, ret_anim_speed, model_list, ghlInfo->aHeader->numFrames))
	{
		assert(*startFrame >= 0 && *startFrame < ghlInfo->aHeader->numFrames);
		assert(*endFrame > 0 && *endFrame <= ghlInfo->aHeader->numFrames);
		assert(*current_frame >= 0.0f && static_cast<int>(*current_frame) < ghlInfo->aHeader->numFrames);
		return qtrue;
	}
...
qboolean G2_Pause_Bone_Anim(const CGhoul2Info* ghlInfo, boneInfo_v& blist, const char* boneName, const int current_time)
{
	...
	if (index != -1)
	{
		...
		if (blist[index].pauseTime)
		{
			...
			G2_Get_Bone_Anim(ghlInfo, blist, boneName, blist[index].pauseTime, &current_frame, &startFrame, &endFrame, &flags, &animSpeed, NULL, 0);
			// reset start time so we are actually on this frame right now
			G2_Set_Bone_Anim(ghlInfo, blist, boneName, startFrame, endFrame, flags, animSpeed, current_time, current_frame, 0);
			// no pausing anymore
			blist[index].pauseTime = 0;
		}
				...
				if (anim_size)
				{
						...
						animSpeed < 0.0f && new_frame_g < endFrame + 1)
					{
						...
						if (blist[i].flags & BONE_ANIM_OVERRIDE_LOOP)
						{
							...
							blist[i].startTime = current_time - static_cast<int>(frame_time / animSpeed * 50.0f);
...
static qboolean G2_Set_Bone_Anim_No_BS(const CGhoul2Info& ghoul2, boneInfo_v& blist, const char* boneName, const int arg_start_frame, const int arg_end_frame, const int flags, const float animSpeed)
{
	int			index = G2_Find_Bone_Rag(&ghoul2, blist, boneName);
	int			mod_flags = flags;

	const int startFrame = arg_start_frame;
	const int endFrame = arg_end_frame;
	...
	if (index != -1)
	{
		...
		//		blist[index].lastTime = blist[index].startTime = (current_time - (((set_frame - (float)startFrame) * 50.0)/ animSpeed));
		blist[index].flags &= ~(BONE_ANIM_TOTAL);
		blist[index].flags |= mod_flags;
	...
	if (index != -1)
	{
		...
		blist[index].pauseTime = 0;
		//		blist[index].boneMap = NULL;
		//		blist[index].lastTime = blist[index].startTime = (current_time - (((set_frame - (float)startFrame) * 50.0f)/ animSpeed));
		blist[index].flags &= ~(BONE_ANIM_TOTAL);
		blist[index].flags |= mod_flags;
...
static char* G2_Get_Bone_Name(const CGhoul2Info* ghlInfo, const boneInfo_v& blist, const int bone_num)
{
	const mdxaSkelOffsets_t* offsets = reinterpret_cast<mdxaSkelOffsets_t*>((byte*)ghlInfo->aHeader + sizeof(mdxaHeader_t));
...
static void Rag_Trace(trace_t* results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, const int pass_entity_num, const int contentmask)
{
#ifdef _DEBUG
