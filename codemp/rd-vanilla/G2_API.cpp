...
int G2API_InitGhoul2Model(CGhoul2Info_v** ghoul2Ptr, const char* file_name, const int modelIndex, const qhandle_t customSkin, const qhandle_t customShader, const int model_flags, const int lod_bias)
{
	int model;
...
qboolean G2API_SetSurfaceOnOff(CGhoul2Info_v& ghoul2, const char* surface_name, const int flags)
{
	...
	if (G2_SetupModelPointers(ghlInfo))
	{
		...
		return G2_SetSurfaceOnOff(ghlInfo, ghlInfo->mSlist, surface_name, flags);
	}
	return qfalse;
}
...
int G2API_GetSurfaceOnOff(CGhoul2Info* ghlInfo, const char* surface_name)
{
	if (G2_SetupModelPointers(ghlInfo))
	{
		return G2_IsSurfaceOff(ghlInfo, ghlInfo->mSlist, surface_name);
	}
	return -1;
}
...
int G2API_GetSurfaceRenderStatus(CGhoul2Info_v& ghoul2, const int modelIndex, const char* surface_name)
{
	...
	if (G2_SetupModelPointers(ghlInfo))
	{
		return G2_IsSurfaceRendered(ghlInfo, surface_name, ghlInfo->mSlist);
	}
	return -1;
}
...
qboolean G2API_HasGhoul2ModelOnIndex(CGhoul2Info_v** ghlRemove, const int modelIndex)
{
	...
	if (!ghlInfo.size() || ghlInfo.size() <= modelIndex || ghlInfo[modelIndex].mmodel_index == -1)
	{
		return qfalse;
	}
...
qboolean G2API_RemoveGhoul2Model(CGhoul2Info_v** ghlRemove, const int modelIndex)
{
	...
	if (ghlInfo.size() > modelIndex)
	{
		...
		ghlInfo[modelIndex].mmodel_index = -1;
...
qboolean G2API_RemoveGhoul2Models(CGhoul2Info_v** ghlRemove)
{
	...
	{
		if (ghlInfo[modelIndex].mmodel_index == -1)
		{
			continue;
		}
#ifdef _G2_GORE
...
qboolean G2API_SetBoneAnimIndex(CGhoul2Info* ghlInfo, const int index, const int astart_frame, const int aend_frame, const int flags, const float animSpeed, const int acurrent_time, const float aset_frame, const int blendTime)
{
	...
	int endFrame = aend_frame;
	int startFrame = astart_frame;
	...
	if (res)
	{
		...
		return G2_Set_Bone_Anim_Index(ghlInfo->mBlist, index, startFrame, endFrame, flags, animSpeed, acurrent_time, setFrame, blendTime, ghlInfo->aHeader->numFrames);
	}
	return qfalse;
}
...
qboolean G2API_SetBoneAnim(CGhoul2Info_v& ghoul2, const int modelIndex, const char* boneName, const int astart_frame, const int aend_frame, const int flags, const float animSpeed, const int acurrent_time, const float aset_frame, const int blendTime)
{
	int endFrame = aend_frame;
	int startFrame = astart_frame;
	...
	if (ghoul2.size() > modelIndex)
	{
		...
		if (res)
		{
			// ensure we flush the cache
			ghlInfo->mSkelFrameNum = 0;
			return G2_Set_Bone_Anim(ghlInfo, ghlInfo->mBlist, boneName, startFrame, endFrame, flags, animSpeed, acurrent_time, set_frame, blendTime);
		}
	}
	return qfalse;
}
...
qboolean G2API_GetBoneAnim(CGhoul2Info_v& ghoul2, const int modelIndex, const char* boneName, const int current_time, float* current_frame, int* startFrame, int* endFrame, int* flags, float* animSpeed, int* model_list)
{
	...
	if (G2_SetupModelPointers(ghlInfo))
	{
		...
		const qboolean ret = G2_Get_Bone_Anim(ghlInfo, ghlInfo->mBlist, boneName, aCurrentTime, current_frame, startFrame, endFrame, flags, animSpeed, model_list, ghlInfo->mmodel_index);
#ifdef _DEBUG
		/*
		assert(*endFrame>0);
		assert(*endFrame<100000);
...
qboolean G2API_SetBoneAnglesIndex(CGhoul2Info* ghlInfo, const int index, const vec3_t angles, const int flags, const Eorientations yaw, const Eorientations pitch, const Eorientations roll, qhandle_t* model_list, const int blendTime, const int acurrent_time)
{
	...
	if (res)
	{
		// ensure we flush the cache
		ghlInfo->mSkelFrameNum = 0;
		return G2_Set_Bone_Angles_Index(ghlInfo->mBlist, index, angles, flags, yaw, pitch, roll, model_list, ghlInfo->mmodel_index, blendTime, acurrent_time);
	}
	return qfalse;
}
...
qboolean G2API_SetBoneAngles(CGhoul2Info_v& ghoul2, const int modelIndex, const char* boneName, const vec3_t angles, const int flags, const Eorientations up, const Eorientations left, const Eorientations forward, qhandle_t* model_list, const int blendTime, const int acurrent_time)
{
	if (ghoul2.size() > modelIndex)
	{
		...
		if (res)
		{
			// ensure we flush the cache
			ghlInfo->mSkelFrameNum = 0;
			return G2_Set_Bone_Angles(ghlInfo, ghlInfo->mBlist, boneName, angles, flags, up, left, forward, model_list, ghlInfo->mmodel_index, blendTime, acurrent_time);
		}
	}
	return qfalse;
}
...
qboolean G2API_SetBoneAnglesMatrixIndex(CGhoul2Info* ghlInfo, const int index, const mdxaBone_t& matrix, const int flags, qhandle_t* model_list, const int blendTime, const int acurrent_time)
{
	if (G2_SetupModelPointers(ghlInfo))
	{
		// ensure we flush the cache
		ghlInfo->mSkelFrameNum = 0;
		return G2_Set_Bone_Angles_Matrix_Index(ghlInfo->mBlist, index, matrix, flags, model_list, ghlInfo->mmodel_index, blendTime, acurrent_time);
	}
	return qfalse;
}

qboolean G2API_SetBoneAnglesMatrix(CGhoul2Info* ghlInfo, const char* boneName, const mdxaBone_t& matrix, const int flags, const qhandle_t* model_list, const int blendTime, const int acurrent_time)
{
	if (G2_SetupModelPointers(ghlInfo))
	{
		// ensure we flush the cache
		ghlInfo->mSkelFrameNum = 0;
		return G2_Set_Bone_Angles_Matrix(ghlInfo->mFileName, ghlInfo->mBlist, boneName, matrix, flags, model_list, ghlInfo->mmodel_index, blendTime, acurrent_time);
	}
	return qfalse;
}
...
int G2API_AddBoltSurfNum(CGhoul2Info* ghlInfo, const int surf_index)
{
	if (G2_SetupModelPointers(ghlInfo))
	{
		return G2_Add_Bolt_Surf_Num(ghlInfo, ghlInfo->mBltlist, ghlInfo->mSlist, surf_index);
	}
	return -1;
}
...
static qboolean G2API_GetBoltMatrix_SPMethod(CGhoul2Info_v& ghoul2, const int modelIndex, const int bolt_index, mdxaBone_t* matrix, const vec3_t angles, const vec3_t position, const int frame_num, qhandle_t* model_list, const vec3_t scale)
{
...
int G2API_GetSurfaceIndex(CGhoul2Info* ghlInfo, const char* surface_name)
{
	if (G2_SetupModelPointers(ghlInfo))
	{
		return G2_GetSurfaceIndex(ghlInfo, surface_name);
	}
	return -1;
}
...
qboolean G2_TestModelPointers(CGhoul2Info* ghlInfo) // returns true if the model is properly set up
{
	...
	if (ghlInfo->mmodel_index != -1)
	{
		...
		if (ghlInfo->currentModel)
		{
			...
			{
				if (ghlInfo->current_modelSize)
				{
					if (ghlInfo->current_modelSize != ghlInfo->currentModel->mdxm->ofsEnd)
					{
						Com_Error(ERR_DROP, "Ghoul2 model was reloaded and has changed, map must be restarted.\n");
					}
				}
				ghlInfo->current_modelSize = ghlInfo->currentModel->mdxm->ofsEnd;
...
qboolean G2_SetupModelPointers(CGhoul2Info* ghlInfo) // returns true if the model is properly set up
{
	...
	//	G2WARNING(ghlInfo->mmodel_index != -1,"Setup request on non-used info slot?");
	if (ghlInfo->mmodel_index != -1)
	{
		...
		if (ghlInfo->currentModel)
		{
			...
			if (ghlInfo->currentModel->mdxm)
			{
				...
				ghlInfo->current_modelSize = ghlInfo->currentModel->mdxm->ofsEnd;
				G2ERROR(ghlInfo->current_modelSize, va("Zero sized Model? (glm) %s", ghlInfo->mFileName));
