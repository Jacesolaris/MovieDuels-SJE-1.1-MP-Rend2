...
int G2_GetBoneDependents(CGhoul2Info& ghoul2, const int bone_num, int* temp_dependents, int max_dep)
{
	...
	const auto skel = (mdxaSkel_t*)((byte*)boneCache.header + sizeof(mdxaHeader_t) + offsets->offsets[bone_num]);
	int i;
	int ret = 0;
	...
	mdxaBone_t*& ret_basepose, mdxaBone_t*& ret_basepose_inv)
{
	...
	const auto offsets = (mdxaSkelOffsets_t*)((byte*)boneCache.header + sizeof(mdxaHeader_t));
	const auto skel = (mdxaSkel_t*)((byte*)boneCache.header + sizeof(mdxaHeader_t) + offsets->offsets[bone_num]);
	Multiply_3x4Matrix(&bolt, (mdxaBone_t*)&boneCache.Eval(bone_num), &skel->BasePoseMat); // DEST FIRST ARG
	ret_basepose = &skel->BasePoseMat;
	ret_basepose_inv = &skel->BasePoseMatInv;
...
static int G2_ComputeLOD(trRefEntity_t* ent, const model_t* currentModel, int lod_bias)
{
	float flod;
...
static void UnCompressBone(float mat[3][4], const int iBoneIndex, const mdxaHeader_t* p_mdxa_header, const int i_frame)
{
...
static void G2_SetUpBolts(mdxaHeader_t* header, CGhoul2Info& ghoul2, const mdxaBone_v& bone_ptr, boltInfo_v& bolt_list)
{
	const auto offsets = (mdxaSkelOffsets_t*)((byte*)header + sizeof(mdxaHeader_t));
...
static void G2_TransformGhoulBones(boneInfo_v& rootBoneList, const mdxaBone_t& rootMatrix, CGhoul2Info& ghoul2, const int time,
...
static void G2_ProcessSurfaceBolt(const mdxaBone_v& bone_ptr, mdxmSurface_t* surface, const int boltNum, boltInfo_v& boltList,
...
static void G2_ProcessGeneratedSurfaceBolts(CGhoul2Info& ghoul2, const mdxaBone_v& bone_ptr, model_t* mod_t)
{
#ifdef G2_PERFORMANCE_ANALYSIS
...
static void ProcessModelBoltSurfaces(const int surfaceNum, surfaceInfo_v& root_s_list,
...
// We've come across a surface that's designated as a bolt surface, process it and put it in the appropriate bolt place
static void G2_ProcessSurfaceBolt2(CBoneCache& boneCache, const mdxmSurface_t* surface, int boltNum, boltInfo_v& boltList,
...
bool G2_NeedsRecalc(CGhoul2Info* ghlInfo, const int frame_num)
{
		...
		ghlInfo->mBoneCache->mod != ghlInfo->currentModel)
	{
		...
		if (ghlInfo->entityNum != ENTITYNUM_NONE &&
			G2API_OverrideServerWithClientData(ghlInfo, 0))
