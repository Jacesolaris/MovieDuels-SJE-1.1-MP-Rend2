...
void trap_Print(const char* fmt) {
	Q_syscall(UI_PRINT, fmt);
}
void trap_Error(const char* fmt) {
	Q_syscall(UI_ERROR, fmt);
	exit(1);
}
...
qboolean trap_G2API_GetBoltMatrix(void* ghoul2, const int modelIndex, const int bolt_index, mdxaBone_t* matrix, const vec3_t angles, const vec3_t position, const int frameNum, qhandle_t* model_list, vec3_t scale) {
	return Q_syscall(UI_G2_GETBOLT, ghoul2, modelIndex, bolt_index, matrix, angles, position, frameNum, model_list, scale);
}
qboolean trap_G2API_GetBoltMatrix_NoReconstruct(void* ghoul2, const int modelIndex, const int bolt_index, mdxaBone_t* matrix, const vec3_t angles, const vec3_t position, const int frameNum, qhandle_t* model_list, vec3_t scale) {
	return Q_syscall(UI_G2_GETBOLT_NOREC, ghoul2, modelIndex, bolt_index, matrix, angles, position, frameNum, model_list, scale);
}
qboolean trap_G2API_GetBoltMatrix_NoRecNoRot(void* ghoul2, const int modelIndex, const int bolt_index, mdxaBone_t* matrix, const vec3_t angles, const vec3_t position, const int frameNum, qhandle_t* model_list, vec3_t scale) {
	return Q_syscall(UI_G2_GETBOLT_NOREC_NOROT, ghoul2, modelIndex, bolt_index, matrix, angles, position, frameNum, model_list, scale);
}
...
qboolean trap_G2API_SetSkin(void* ghoul2, const int modelIndex, const qhandle_t customSkin, const qhandle_t render_skin) {
	return Q_syscall(UI_G2_SETSKIN, ghoul2, modelIndex, customSkin, render_skin);
}
...
qboolean trap_G2API_SetBoneAngles(void* ghoul2, int modelIndex, const char* boneName, const vec3_t angles, const int flags, const int up, const int right, const int forward, qhandle_t* model_list, int blendTime, int current_time) {
	return Q_syscall(UI_G2_ANGLEOVERRIDE, ghoul2, modelIndex, boneName, angles, flags, up, right, forward, model_list, blendTime, current_time);
}
qboolean trap_G2API_SetBoneAnim(void* ghoul2, const int modelIndex, const char* boneName, const int startFrame, const int endFrame, const int flags, const float animSpeed, const int current_time, const float setFrame, const int blendTime) {
	return Q_syscall(UI_G2_PLAYANIM, ghoul2, modelIndex, boneName, startFrame, endFrame, flags, PASSFLOAT(animSpeed), current_time, PASSFLOAT(setFrame), blendTime);
}
qboolean trap_G2API_GetBoneAnim(void* ghoul2, const char* boneName, const int current_time, float* current_frame, int* startFrame, int* endFrame, int* flags, float* animSpeed, int* model_list, const int modelIndex) {
	return Q_syscall(UI_G2_GETBONEANIM, ghoul2, boneName, current_time, current_frame, startFrame, endFrame, flags, animSpeed, model_list, modelIndex);
}
qboolean trap_G2API_GetBoneFrame(void* ghoul2, const char* boneName, const int current_time, float* current_frame, int* model_list, const int modelIndex) {
	return Q_syscall(UI_G2_GETBONEFRAME, ghoul2, boneName, current_time, current_frame, model_list, modelIndex);
}
void trap_G2API_GetGLAName(void* ghoul2, int modelIndex, char* fillBuf) {
	Q_syscall(UI_G2_GETGLANAME, ghoul2, modelIndex, fillBuf);
}
int trap_G2API_CopyGhoul2Instance(void* g2_from, void* g2To, int modelIndex) {
	return Q_syscall(UI_G2_COPYGHOUL2INSTANCE, g2_from, g2To, modelIndex);
}
...
qboolean trap_G2API_SetRootSurface(void* ghoul2, const int modelIndex, const char* surface_name) {
	return Q_syscall(UI_G2_SETROOTSURFACE, ghoul2, modelIndex, surface_name);
}
