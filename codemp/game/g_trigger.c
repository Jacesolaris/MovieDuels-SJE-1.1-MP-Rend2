...
void multi_trigger(gentity_t* ent, gentity_t* activator)
{
	...
	if (level.gametype == GT_MOVIEDUELS_SIEGE && ent->genericValue1)
	{
		halt_trigger = qtrue;
...
void Touch_Multi(gentity_t* self, gentity_t* other, trace_t* trace)
{
	...
	if (self->spawnflags & 4)
	{
		...
		if (self->genericValue7)
		{
			...
			{
				//start the hack
				if (other->client->ps.saberHolstered == 1 && other->client->saber[1].model[0])
				{//turn off second saber
					G_Sound(other, CHAN_WEAPON, other->client->saber[1].soundOff);
				}
				else if (other->client->ps.saberHolstered == 0)
				{//turn off first
					G_Sound(other, CHAN_WEAPON, other->client->saber[0].soundOff);
				}
...
void hurt_touch(gentity_t* self, gentity_t* other, trace_t* trace)
{
	...
	if (level.gametype == GT_MOVIEDUELS_SIEGE && self->team && self->team[0])
	{
