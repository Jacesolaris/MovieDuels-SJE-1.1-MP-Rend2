...
#if defined(_GAME)
	trap->G2API_GetBoltMatrix(ghoul2, 0, bolt_index, &boltMatrix, ranc_angles, ranc_origin, time, model_list, model_scale);
#elif defined(_CGAME)
	trap->G2API_GetBoltMatrix(ghoul2, 0, bolt_index, &boltMatrix, ranc_angles, ranc_origin, time, model_list, model_scale);
#endif
	...
	const int return_size)
{
...
#endif
	{
		...
		Q_strncpyz(returnSurfName, rootSurfName, return_size);
		return qtrue;
	}
	//check variants
	for (int i = 0; i < MAX_VARIANTS; i++)
	{
		Com_sprintf(returnSurfName, return_size, "%s%c", rootSurfName, 'a' + i);
#if defined(_GAME)
