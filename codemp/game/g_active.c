...
qboolean WP_SaberStyleValidForSaber(const saberInfo_t* saber1, const saberInfo_t* saber2, int saberHolstered, int saber_anim_level);
...
static void G_UpdateClientBroadcasts(gentity_t* self)
{
	...
	for (i = 0, other = g_entities; i < MAX_CLIENTS; i++, other++)
	{
		...
		if (level.gametype == GT_MOVIEDUELS_JEDIMASTER && self->client->ps.isJediMaster)
		{
			...
			else
			{
				if (PM_WalkingAnim(ent->client->ps.legsAnim) || PM_RunningAnim(ent->client->ps.legsAnim))
				{
					...
					case SS_TAVION:
						...
						if (ent->client->pers.botclass == BCLASS_VADER || (saber1 && saber1->type == SABER_VADER))
						{
							NPC_SetAnim(ent, SETANIM_TORSO, BOTH_VADERTAUNT, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
						}
						else
					...
					case SS_MEDIUM:
						...
						else if (saber1 && saber1->type == SABER_VADER || ent->client->pers.botclass == BCLASS_VADER)
						{
							NPC_SetAnim(ent, SETANIM_TORSO, BOTH_VADERTAUNT, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
						}
						else
					...
					case SS_STRONG:
					case SS_DESANN:
						if (saber1 && saber1->type == SABER_VADER || ent->client->pers.botclass == BCLASS_VADER || ent->client->pers.botclass == BCLASS_DESANN)
						{
							NPC_SetAnim(ent, SETANIM_TORSO, BOTH_VADERTAUNT, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
						}
						else
...
static void ClientThink_real(gentity_t* ent)
{
	...
	if (!isFollowing)
	{
		...
		{ //with two sabs always use akimbo style
			...
			{
				if (!WP_SaberStyleValidForSaber(&client->saber[0], &client->saber[1], client->ps.saberHolstered, client->ps.fd.saber_anim_level))
	...
	if (killJetFlags)
	{
		...
		client->ps.eFlags &= ~EF3_JETPACK_HOVER;
	}
...
#define	MIN_NPC_SPEED	16.0f
	...
	else if (level.gametype == GT_MOVIEDUELS_MISSIONS && ent->client->ps.weapon == WP_MELEE)
	{
		if (ucmd->buttons & BUTTON_GRAPPLE && !(ucmd->buttons & BUTTON_WALKING) && !
			PM_RunningAnim(ent->client->ps.legsAnim) && !PM_WalkingAnim(ent->client->ps.legsAnim)
			&& ent->client->ps.pm_type != PM_DEAD
		...
		&& ent->s.eType != ET_NPC)
	{
		...
		if (level.time > client->respawnTime && !gDoSlowMoDuel)
		{
			...
			else if (level.gametype == GT_MOVIEDUELS_SIEGE && g_siegeRespawn.integer)
			{
				//wave respawning on
				forceRes = 1;
			}
...
{
	if (self->client &&
		(!(self->client->ps.fd.forcePowersActive & 1 << FP_SEE)) || //sight is not on OR
		(self->client->ps.fd.forcePowersActive & 1 << FP_SEE && self->client->ps.fd.forcePowerLevel[FP_SEE] < FORCE_LEVEL_2)) //sight is on but lower than 3
