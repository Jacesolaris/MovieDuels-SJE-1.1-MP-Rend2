...
static void CG_DrawHaqrBar(const float chX, const float chY, const float chW, const float chH)
{
	...
	//now draw the part to show how much health there is in the color specified
	CG_FillRect(x + 1.0f, y + 1.0f, percent - 1.0f, HACK_HEIGHT - 1.0f, a_color);

	//then draw the other part greyed out
	CG_FillRect(x + percent, y + 1.0f, HACK_WIDTH - percent - 1.0f, HACK_HEIGHT - 1.0f, c_color);
...
static void CG_DrawCrosshair(vec3_t world_point, const int ch_ent_valid)
{
	...
	{
		CG_DrawHealthBar(cross_ent, chX, chY, w, h);
		chY += HEALTH_HEIGHT * 2;
	}
	else if (cross_ent && cross_ent->currentState.number < MAX_CLIENTS)
	{
		if (cgs.gametype == GT_MOVIEDUELS_SIEGE)
		{
			CG_DrawSiegeInfo(cross_ent, chX, chY, w, h);
			chY += HEALTH_HEIGHT * 4;
		}
...
static qboolean CG_CalcVehiclemuzzle_point(const int entityNum, vec3_t start, vec3_t d_f, vec3_t d_rt, vec3_t d_up)
{
...
static void CG_CalcEWebmuzzle_point(centity_t* cent, vec3_t start, vec3_t d_f, vec3_t d_rt, vec3_t d_up)
{
