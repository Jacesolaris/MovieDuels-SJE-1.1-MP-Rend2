	...
	if (entities_initialized && ci->ghoul2Model && old_ghoul2 != ci->ghoul2Model)
	{
		...
		if (anim)
		{
			...
			trap->G2API_SetBoneAnim(ci->ghoul2Model, 0, "model_root", first_frame, anim->firstFrame + anim->numFrames,
				flags, animSpeed, cg.time, set_frame, 150);
...
*/
static void CG_PlayerAnimEvents(const int anim_file_index, const int event_file_index, const qboolean torso,
	...
	const int blendTime, const int current_time)
{
	...
	trap->G2API_SetBoneAngles(ghoul2, modelIndex, boneName, angles, flags, up, right, forward, model_list,
...
#elif 1 //with my new method of doing things I want it to continue the anim
		{
				...
				&flags, &animSpeed, cgs.game_models, 0))
			{
				...
				trap->G2API_SetBoneAnim(cent->ghoul2, 0, "lower_lumbar", current_frame, current_frame + 1, flags,
					animSpeed, cg.time, current_frame, blendTime);
				trap->G2API_SetBoneAnim(cent->ghoul2, 0, "model_root", current_frame, current_frame + 1, flags,
					animSpeed,
					cg.time, current_frame, blendTime);
				trap->G2API_SetBoneAnim(cent->ghoul2, 0, "Motion", current_frame, current_frame + 1, flags, animSpeed,
					cg.time, current_frame, blendTime);
			}
		}
#endif
...
static void CG_ForcePushBodyBlur(centity_t* cent)
{
	...
	for (int i = 0; cg_pushBoneNames[i]; i++)
	{
		...
		trap->G2API_GetBoltMatrix(cent->ghoul2, 0, lhand, &boltMatrix, cent->turAngles, cent->lerpOrigin, cg.time, cgs.game_models, cent->modelScale);
...
=================
*/
static int CG_LightVerts(vec3_t normal, const int numVerts, polyVert_t* verts)
{
...
static void RGB_AdjustPimpSaberColor(clientInfo_t* ci, vec3_t color, const int n)
{
	int time;
...
static void CG_DoCloakedSaber(vec3_t origin, vec3_t dir, float length, float length_max, float radius, saber_colors_t color,
...
static void CG_DoSaber(vec3_t origin, vec3_t dir, float length, float length_max, float radius, saber_colors_t color, int rfx,
...
static void CG_DoUnstableSaber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float length_max,
	float radius, saber_colors_t color, int rfx, qboolean do_light, int cnum, int bnum)
{
	...
	effectradius = (radius * 1.6f * v1 + Q_flrand(-1.0f, 1.0f) * 0.1f) * radiusmult * cg_SFXSabersGlowSizeUS.value;
	coreradius = (radius * 0.4f * v2 + Q_flrand(-1.0f, 1.0f) * 0.1f) * radiusmult * cg_SFXSabersCoreSizeUS.value;
...
static void CG_DoSaberUnstable(vec3_t origin, vec3_t dir, float length, float length_max, float radius, saber_colors_t color,
	int rfx, qboolean do_light, int cnum, int bnum)
{
	vec3_t mid;
	qhandle_t blade = 0, glow = 0;
	...
	float radius, saber_colors_t color, int rfx, qboolean do_light, int cnum, int bnum)
{
	...
	case SABER_YELLOW:
		...
		blade = cgs.media.ep7yellowSaberCoreShader;
		cgs.media.sfxSaberEndShader = trap->R_RegisterShader("SFX_Sabers/saber_end");
		cgs.media.sfxSaberTrailShader = trap->R_RegisterShader("SFX_Sabers/saber_trail");
		ignite = cgs.media.yellowIgniteFlare;
		break;
...
//--------------------------------------------------------------
static void CG_GetTagWorldPosition(refEntity_t* model, const char* tag, vec3_t pos, matrix3_t axis)
{
...
static void CG_SaberCompWork(vec3_t start, vec3_t end, centity_t* owner, const int saberNum, const int blade_num)
{
	trace_t trace;
	...
	else
	{
		...
		case 1:
			if (cent->currentState.botclass == BCLASS_UNSTABLESABER || client->saber[saberNum].type == SABER_UNSTABLE
				...
				SABER_ELECTROSTAFF)
			{
				CG_DoUnstableSaber(fx.mVerts[0].origin, fx.mVerts[1].origin, fx.mVerts[2].origin, fx.mVerts[3].origin,
...
static void CG_AddLightningBeam(vec3_t start, vec3_t end)
{
	vec3_t dir, chaos,
		c1, c2,
		v1, v2;
...
static void CG_AddRandomLightning(vec3_t start, vec3_t end)
{
...
#if 0
static void CG_DrawNoForceSphere(centity_t* cent, vec3_t origin, float scale, int shader)
{
	refEntity_t ent;
...
static void CG_CreateSurfaceDebris(centity_t* cent, const int surf_num, const int fx_id, const qboolean throw_part)
{
	...
	if (surf_num > 0)
	{
		surfName = bgToggleableSurfaces[surf_num];
	}
...
static qboolean CG_VehicleShouldDrawShields(const centity_t* veh_cent)
{
...
static void CG_CheckThirdPersonAlpha(const centity_t* cent, refEntity_t* legs)
{
	float alpha = 1.0f;
	int set_flags = 0;
...
{
	return bg_get_torso_anim_point(&cg.predictedPlayerState, cg_entities[cg.predictedPlayerState.clientNum].localAnimIndex);
}
...
SkipTrueView:
	...
	if (cent->currentState.PlayerEffectFlags & 1 << PEF_FLAMING)
	{
	...
	if (cent->currentState.powerups & (1 << PW_SPHERESHIELDED) && cent->currentState.number == cg.snap->ps.clientNum)
	{
		...
		&& health > 1 && !in_camera)
	{
		if (cent->ghoul2)
		{
			...
			trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_head, &boltMatrix, t_ang, cent->lerpOrigin, cg.time, cgs.game_models, cent->modelScale);
