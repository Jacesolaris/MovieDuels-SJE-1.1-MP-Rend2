...
extern qboolean BG_GetRootSurfNameWithVariant(void* ghoul2, const char* rootSurfName, char* returnSurfName, int return_size);
...
void CG_EntityEvent(centity_t* cent, vec3_t position)
{
	...
	case EV_SABER_HIT:
		DEBUGNAME("EV_SABER_HIT");
		{
			...
			int hit_other_fx_id = cgs.effects.mSaberCut_AMD;
	...
	case EV_MISSILE_HIT:
		DEBUGNAME("EV_MISSILE_HIT");
		{
			...
			{ // this client is dueling
				...
				if (es->otherEntityNum != cg.snap->ps.clientNum && es->otherEntityNum != cg.snap->ps.duelIndex)
				{
					doit = 0;
				}
			}
